...
let state = {
  ...
  sitefType: null,
  canalVenda: null,
  versao_sitef: null, // Campos do Passo 1:
  nomeCliente: null,
  cpfCliente: null,
...
function voltar(targetStep) {
  if (targetStep < 1 || targetStep > 6) return; // Restrição de navegação para o passo de seleção SiTef (Passo 3)
...
function setupStep1() {
  ...
  fields.forEach((id) => {
    ...
    if (el) {
      ...
      el.value = state[id] || ""; // 2. Adiciona Listener APENAS para Máscaras e Validações de UI (SEM salvar state)
      ...
      if (id === "nomeEstabelecimento") {
        el.addEventListener("change", (e) => {
          ...
          if ($("codigoEstabelecimento"))
            $("codigoEstabelecimento").value = codigo;
  ...
  if (state.nomeEstabelecimento && $("codigoEstabelecimento")) {
    $("codigoEstabelecimento").value =
      estabelecimentos[state.nomeEstabelecimento] || "";
  }
}
setupStep1();
...
function formatarValorSiTef(valorBruto) {
  if (typeof valorBruto !== "string" && typeof valorBruto !== "number")
...
function normalizeNSU(nsuString) {
  if (typeof nsuString !== "string" || nsuString.trim() === "") return ""; // Remove caracteres não-numéricos, converte para número (removendo zeros à esquerda) // e retorna como string. Se for NaN, retorna string vazia.
...
function formatarDataSescnet(dataString) {
  if (!dataString || dataString.length < 10) return ""; // Supondo formato DD/MM/AAAA
  const parts = dataString.split("/");
  if (parts.length === 3) {
    const [dia, mes, ano] = parts; // Pega apenas os dois últimos dígitos do ano (AA)
    const anoCurto = ano.slice(-2); // Monta a string no formato 4AAMMDD
...
if ($("btnNext1")) {
  $("btnNext1").addEventListener("click", () => {
    ...
    state.dataSolicitacao = $("dataSolicitacao")
    ...
    state.codigoEstabelecimento =
      estabelecimentos[state.nomeEstabelecimento] || null; // 2. Lógica de validação
    ...
    if (!state.caixa || Number(state.caixa) < 1)
      return showModal(
        "Atenção",
        "O número do caixa deve ser um número inteiro positivo."
      );
    if (!state.numeroVenda || Number(state.numeroVenda) < 1)
      return showModal(
        "Atenção",
        "O número da venda deve ser um número inteiro positivo."
      );
    if (!state.nomeEstabelecimento || !state.nomeEstabelecimento.trim())
      return showModal("Atenção", "Selecione o estabelecimento.");
    if (!state.canalVenda) return showModal("Atenção", "Selecione o canal."); // 3. Salva o estado e prossegue
    saveState();
    mostrarStep(2);
  });
}
...
function parseSescnetData(rawData) {
  ...
  const parts = firstLine.split(/\t| {2,}/g).map((p) => p.trim()); // Remove colunas vazias iniciais

  while (parts.length > 0 && parts[0] === "") parts.shift();
  const numColunas = parts.length;
  let partsMapeamento = [...parts]; // LÓGICA DE DETECÇÃO E AJUSTE DE COLUNAS
  ...
  if (numColunas === 15) {
    ...
    outrosPgto = partsMapeamento[7]; // Se a coluna 7 existe, todos os campos de índice 8 em diante na visão do usuário // estão uma posição à frente.
    baseIndex = 1;
  } else if (numColunas === 14) {
    // Se houver 14 colunas, o índice 7 é o 'Total Geral'
    outrosPgto = "";
    baseIndex = 0;
  } // Mapeamento dos índices (Ajustados com base no baseIndex) // Índice 0 a 6: Seq até Multa (são fixos) // Índice 7: Outros Pgto (se existir, é o parts[7]) // Índice 8: Total Geral (parts[7] ou parts[8]) // Índice 9: Tipo Liquidacao (parts[8] ou parts[9])
  ...
  const idx_tid = 13 + baseIndex; // GARANTIA: Preenche o mapeamento para evitar acessos a índices que não existem
  while (partsMapeamento.length < idx_tid + 1) partsMapeamento.push("");

  const nsu_tef_raw = partsMapeamento[idx_nsu_tef]
    ? partsMapeamento[idx_nsu_tef].trim()
    : "";
  const nsu_web_raw = partsMapeamento[idx_nsu_web]
    ? partsMapeamento[idx_nsu_web].trim()
    : ""; // Lógica para determinar o NSU principal (se houver)

  const nsu_sescnet =
    nsu_tef_raw !== "0" && nsu_tef_raw !== ""
      ? nsu_tef_raw
      : nsu_web_raw !== "0" && nsu_web_raw !== ""
      ? nsu_web_raw
      : "";
  const obj = {
    ...
    multa: partsMapeamento[6], // O campo que você vê como 7 (Outros Pgto)
...
if ($("btnNext2")) {
  $("btnNext2").addEventListener("click", () => {
    ...
    if (state.dadosSescnetTable.length === 0) {
      return showModal(
        "Atenção",
        "Cole os dados do SescNet. Não foi possível extrair os dados da tabela."
      );
    }

    const dadosSescnet = state.dadosSescnetTable[0]; // NOVO: VALIDAÇÃO PIX NO SESCNET
    ...
    if (
      tipoLiquidacao.includes(pixCheck) ||
      operacaoContabil.includes(pixCheck)
    ) {
      return showModal(
        "AVISO",
        "Operação informada se trata de um PIX, favor prosseguir com uma transação em CARTÃO"
      );
    }
...
document.getElementById("paste-area").addEventListener("paste", function (e) {
  ...
  this.value = text; // Processa os dados
  state.dadosSescnetTable = parseSescnetData(text); // Mapeamento dos inputs de visualização (AJUSTADO PARA A NOVA ESTRUTURA)

  let inputs = document.querySelectorAll("#step-2 .col-input");
  if (state.dadosSescnetTable.length > 0) {
    const obj = state.dadosSescnetTable[0]; // Array com os valores na ordem exata da tela, incluindo 'outros_pgto'
    const valuesInOrder = [
      obj.seq,
      obj.data_sescnet,
      obj.caixa_sescnet,
...
const sitefWebColumnMap = [
  ...
  { label: "Cód. Autor", index: 14, id: "aut" }, // O resto é mapeado implicitamente
...
if ($("sitefWebInput")) {
  $("sitefWebInput").addEventListener("input", function () {
    ...
    document
      .querySelectorAll("#step-4 .col-input-sitef-web")
    ...
    document.querySelectorAll("#step-4 .label").forEach((labelEl) => {
      ...
      const mapEntry = sitefWebColumnMap.find(
        (item) =>
...
if ($("btnNext4")) {
  $("btnNext4").addEventListener("click", () => {
    ...
    if (dados.length === 0) {
      return showModal(
        "Atenção",
        "Cole a linha tabulada do SiTef Web (Antigo) na caixa de texto."
      );
    } // Verificação relaxada para permitir menos colunas, mas avisa se faltarem as críticas
    if (dados.length < 15)
      showModal(
        "Aviso",
        "Os dados do SiTef Web parecem incompletos (menos de 15 colunas). Verifique a cópia."
      );

    const nsuSitefRaw = dados[4] ? dados[4].trim() : "";
    const nsuSescnetRaw = dadosSescnet.nsu_sescnet
      ? dadosSescnet.nsu_sescnet.trim()
      : ""; // Normaliza ambos os NSUs para comparação
    const nsuSitef = normalizeNSU(nsuSitefRaw);
    const nsuSescnet = normalizeNSU(nsuSescnetRaw); // VALIDAÇÃO PIX NO SITEF WEB
  ...
              Favor revisar as informações`
    ...
    if (!dados[1] || !dados[4] || !dados[14]) {
      return showModal(
        "Atenção",
        "Dados críticos (Data, NSU e/ou Cód. Autor) não encontrados nos dados colados do SiTef Web. Verifique se o relatório está completo."
      );
    }
...
if ($("sitefExpressInput")) {
  $("sitefExpressInput").addEventListener("input", function () {
    ...
    const raw = inputEl.value.trim();
    document
      .querySelectorAll("#step-5 .col-input-sitef")
      .forEach((input) => (input.value = ""));
    ...
    document.querySelectorAll("#step-5 .label").forEach((labelEl) => {
      ...
      if (valor !== undefined) {
        ...
        if (
          inputResultEl &&
          inputResultEl.classList.contains("col-input-sitef")
        ) {
    ...
    saveState(); // Limpa a textarea imediatamente após processar e atualizar o state
...
if ($("btnNext5")) {
  $("btnNext5").addEventListener("click", () => {
    const dadosSescnet = state.dadosSescnetTable[0]; // Valida se o estado foi preenchido

    if (Object.keys(state.dadosSitefExpress).length === 0) {
      return showModal(
        "Atenção",
        "Cole os dados do SiTef Express. Não foi possível extrair os dados. Verifique o formato."
      );
    }
    ...
    if (!state.dadosSitefExpress["Data"] || !state.dadosSitefExpress["NSU"]) {
      return showModal(
        "Atenção",
        "Não foi possível extrair os dados críticos (Data e NSU) do SiTef Express. Verifique a formatação colada."
      );
    } // VALIDAÇÃO PIX NO SITEF EXPRESS
    const tipoProdutoExpress = state.dadosSitefExpress["Tipo Produto"]
      ? state.dadosSitefExpress["Tipo Produto"].trim()
      : "";
    ...
    const nsuSitefRaw = state.dadosSitefExpress["NSU"]
      ? state.dadosSitefExpress["NSU"].trim()
      : "";
    const nsuSescnetRaw = dadosSescnet.nsu_sescnet
      ? dadosSescnet.nsu_sescnet.trim()
      : ""; // Normaliza ambos os NSUs para comparação
...
function montarResumo() {
  ...
  let valorTransacaoNum = 0; // Condições que usam dados Sescnet: E-commerce, POS, ou qualquer outro canal que não seja SiTef, // ou se for SiTef e os dados dele não foram preenchidos (o que não deve acontecer na navegação normal)
  const isSescnetOnly =
    state.canalVenda === "ecommerce" || state.canalVenda === "pos";
  const dadosSescnet = state.dadosSescnetTable[0];
  ...
  if (dadosSescnet) {
    if (isSescnetOnly) {
      ...
      if (state.versao_sitef === "web" && state.dadosSitefWeb.length > 0) {
        ...
        state.versao_sitef === "express" &&
        Object.keys(state.dadosSitefExpress).length > 0
      ) {
        const valorBrutoExpress = state.dadosSitefExpress["Valor"] || "0"; // Tenta extrair o valor numérico da string SiTef Express
        const valorParaParse = valorBrutoExpress
          .replace(/[^\d,]/g, "")
          .replace(",", ".");
        valorTransacaoNum = Number(valorParaParse) || 0; // Formata apenas se for um valor limpo de SiTef. Se for formatado como R$, usa direto
        if (valorBrutoExpress.includes("R$")) {
          valorCardFormatado = valorBrutoExpress;
        } else {
          valorCardFormatado = new Intl.NumberFormat("pt-BR", {
            style: "currency",
            currency: "BRL",
          }).format(valorTransacaoNum);
        }
      } else {
  ...
  const valorCancelar = $("valorCancelar")
    ? $("valorCancelar").value
    : "(pendente)";
...
function limparStepAtual() {
  switch (state.step) {
    case 1: // Limpa inputs do Passo 1 e reseta o state
      document
        .querySelectorAll("#step-1 input, #step-1 select")
        .forEach((input) => (input.value = ""));
      document
        .querySelectorAll("#step-1 select")
      ...
      break;
    case 2: // Limpa dados do Sescnet
      document
        .querySelectorAll("#step-2 .col-input")
        .forEach((input) => (input.value = ""));
      if ($("paste-area")) $("paste-area").value = "";
      state.dadosSescnetTable = [];
      break;
    case 3: // Limpa seleção SiTef, forçando o usuário a re-selecionar ao retornar
      state.sitefType = null;
      state.versao_sitef = null;
      break;
    case 4: // Limpa SiTef Web
      document
        .querySelectorAll("#step-4 .col-input-sitef-web")
        .forEach((input) => (input.value = ""));
  ...
  showModal(
    "Limpeza Concluída",
    `As informações do **Passo ${state.step}** foram apagadas.`
  );
}
saveState();
...
function getUserPrefix() {
  ...
  if (email) {
    ...
    const prefixo = email.split("@")[0]; // Remove a chave do localStorage após o uso (boa prática)
...
async function gerarExcelPreenchido() {
  const valorCancelarRaw = $("valorCancelar")
    ? $("valorCancelar").value.trim()
    : "";
  const valorCancelarNum = parseCurrencyToNumber(valorCancelarRaw); // INICIALIZAÇÃO DE VARIÁVEIS DE CAPA PARA GARANTIR QUE NÃO SEJAM UNDEFINED
  let n_cartao = "";
  let valorTransacaoFinal = "";
  if (!valorCancelarRaw || valorCancelarNum <= 0)
    return showModal(
      "Atenção",
      "Informe um valor a cancelar válido e maior que zero."
    );

  let valorTransacaoNum = 0;
  const dadosSescnet = state.dadosSescnetTable[0]; // 1. Lógica para determinar o valor de referência e variáveis de CAPA (n_cartao, valor)
  if (state.canalVenda === "sitef") {
    if (state.versao_sitef === "web" && state.dadosSitefWeb.length > 0) {
      const valorBrutoSitef = $("valor_sitef") ? $("valor_sitef").value : "0";
  ...
  if (valorCancelarNum > valorTransacaoNum) {
    const formatadoCancelar = new Intl.NumberFormat("pt-BR", {
      style: "currency",
      currency: "BRL",
    }).format(valorCancelarNum);
    const formatadoTransacao = new Intl.NumberFormat("pt-BR", {
      style: "currency",
      currency: "BRL",
    }).format(valorTransacaoNum);
    return showModal(
  ...
  const workbook = new ExcelJS.Workbook(); // Variáveis para IDs das Imagens
  ...
  try {
    ...
    if (!sescResp.ok)
      ...
      ); // Converte os buffers das imagens // REMOVIDOS: sescnetBuffer, sitefAntigoBuffer, sitefNovoBuffer
    const [buffer, sescBuffer] = await Promise.all([
      templateResp.arrayBuffer(),
      sescResp.arrayBuffer(),
    ]); // Carrega o template
    await workbook.xlsx.load(buffer); // --- 2. ADICIONAR IMAGENS AO WORKBOOK (Obter IDs) --- // REMOVIDO: logoSescnetId
    logoSescId = workbook.addImage({
      base64: arrayBufferToBase64(sescBuffer, "image/png"),
      extension: "png",
    }); // REMOVIDOS: sitefAntigoId, sitefNovoId
  } catch (e) {
    showLoading(false);
    return showModal(
      "Erro",
      "Erro carregando recursos e template.xlsx<br>" + (e.message || e)
    );
  }

  try {
    const sheetCapa = workbook.getWorksheet("CAPA");
    const sheetTransacao = workbook.getWorksheet("TRANSACAO"); // --- 3. INSERÇÃO DAS IMAGENS NA PLANILHA --- // /assets/logo-sesc.png em CAPA H9 // Ajustado para ocupar uma área (H9 a I10, por exemplo) para ser visível
    if (logoSescId) sheetCapa.addImage(logoSescId, "H9:N18"); // ======================================================= // PREENCHIMENTO DA PLANILHA CAPA // ======================================================= // Preenche a CAPA

    sheetCapa.getCell("E16").value = state.nomeCliente || "";
    sheetCapa.getCell("E17").value = state.cpfCliente || "";
    sheetCapa.getCell("E14").value =
      estabelecimentos[state.nomeEstabelecimento] || ""; // Converte a data do formato yyyy-mm-dd para objeto Date

    const [year, month, day] = (state.dataSolicitacao || "")
      .split("-")
      .map(Number); // >> TRATAMENTO DA DATA DO SITEF WEB/EXPRESS PARA O EXCEL (E10) <<
    ...
    sheetCapa.getCell("E11").value = Number(state.caixa || 0); // E23: Valor a Cancelar (Número puro)
    ...
    const now = new Date(); // Formata o horário (HH:MM:SS) e a data (DD/MM/AAAA)
    ...
    const logText = `Planilha gerada via site por ${usuario} às ${horario}, dia ${dataGerada}`; // CÉLULA ALTERADA DE E25 PARA I23
    sheetCapa.getCell("I23").value = logText; // ======================================================= // PREENCHIMENTO DA PLANILHA TRANSACAO // ======================================================= // 1. SESCNET (C3)
    if (dadosSescnet) {
      // Gera o array de valores na ordem da planilha (C3, D3, E3, etc.)
      const valuesInOrder = [
        dadosSescnet.seq,
        dadosSescnet.data_sescnet,
        dadosSescnet.caixa_sescnet,
        dadosSescnet.valor_sescnet,
        dadosSescnet.abatimento,
        dadosSescnet.juros,
        dadosSescnet.multa,
        dadosSescnet.outros_pgto, // Índice 7: Outros Pgto
        dadosSescnet.total_geral,
        dadosSescnet.tipo_liquidacao,
        dadosSescnet.operacao_contabil,
        dadosSescnet.parcelas,
        dadosSescnet.nsu_tef,
        dadosSescnet.nsu_web,
        dadosSescnet.tid_sescnet,
      ];
      const row = sheetTransacao.getRow(3); // Coluna C é a de índice 3 no ExcelJS
      let colIndex = 3; // Preenche C3, D3, E3, F3, ...
    ...
    showLoading(false); // LIMPA O ESTADO E REINICIA
    loadState();
    showModal(
      "Sucesso",
      "Arquivo gerado com sucesso! O sistema será reiniciado.",
      {
        onOk: () => {
